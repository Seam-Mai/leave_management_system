...
namespace leave_management_system.Dashboard.Manager
{
    public partial class ManagerView : UserControl
    {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        foreach (DataGridViewRow dgvRow in MdataGridView1.Rows)
                        {
            ...
            using (SqlConnection con = new SqlConnection(ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                   ...
                   MdataGridView1.Rows.Clear();
                    ...
                    MlblPage.Text = $"Search results: {dt.Rows.Count} record(s)";
                }
            }
        }
        ...
        {
            if (e.RowIndex >= 0 && MdataGridView1.Columns[e.ColumnIndex].Name == "Action")
            {
                int requestId = Convert.ToInt32(MdataGridView1.Rows[e.RowIndex].Cells["request_id"].Value);
        ...
        private void UpdatePaginationLabels()
        {
            ...
            MlblPage.Text = $"Page {currentPage} of {totalPages}";
            MbtnPrevious.Enabled = currentPage > 1;
            MbtnNext.Enabled = currentPage < totalPages;
        }
    }
