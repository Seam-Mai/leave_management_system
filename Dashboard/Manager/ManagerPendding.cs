...
{
    public partial class ManagerPendding : UserControl
    {
        public ManagerPendding()
        {
            InitializeComponent();
            MLoadPendingLeaveRequests();
        }

        private string ConnectionString = @"Server=SEAM-MAI\MSIDB;Database=leave_system;Integrated Security=true;Encrypt=False";

        private void MLoadPendingLeaveRequests()
        {
            try
            {
        ...
        private void HRdataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (MdataGridView1.Columns[e.ColumnIndex].Name == "Action")
                {
                    int requestId = Convert.ToInt32(MdataGridView1.Rows[e.RowIndex].Cells["ID"].Value);
        ...
        private void MPending_Click(object sender, EventArgs e)
        {
            RefreshData();
